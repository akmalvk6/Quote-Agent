...
class SupervisorAgent:
    def __init__(
        ...
        self.product_discovery = ProductDiscovery(data_dir / "products.csv", data_dir / "vector_store")
    ...
    def run(self, user_request: str, customer_type: str = "regular") -> Dict[str, Any]:
        with self.telemetry.span("supervisor_agent", request=user_request):
            ...
            # --- Product discovery with confidence tiers ---
            discovery = ProductDiscoveryAgent().discover(
                parsed["product"], self.repository, self.product_discovery
            )
            if not discovery["ok"]:
                return discovery

            resolved_product_name = discovery["product"]["name"]
...
class RetrieverAgent:
    """Extracts structured fields from a user's quote request using LLM."""

    def __init__(self):
        self._extractor = LLMExtractor()

    def parse_request(self, user_request: str) -> Dict[str, Any]:
        result: QuoteRequest | None = self._extractor.extract(user_request)
        if result is None:
            return {
                "ok": False,
                "agent": "RetrieverAgent",
                "message": "Could not parse request. Please include quantity, product, and customer, for example: 50 Office Chairs for ABC Corp.",
            }
        return {
            "ok": True,
            "agent": "RetrieverAgent",
...
class ProductDiscoveryAgent:
    """Wraps product discovery with confidence-tiered matching."""
    ...
    def discover(
        ...
        if product:
            return {"ok": True, "agent": "ProductDiscoveryAgent", "product": product, "discovery_used": False, "metadata": None}

        result = product_discovery.find_or_match(product_name, repository)
        ...
        # ask_user or no_match
        return {
            "ok": False,
            "agent": "ProductDiscoveryAgent",
            "message": result.get("message", "No matching product found."),
...
class QuoteGeneratorAgent:
    ...
    def generate(
        ...
        if discovery_metadata:
            quote["requested_product"] = discovery_metadata.get("requested_product", "")
            quote["matched_product"] = discovery_metadata.get("matched_product", "")
            quote["match_score"] = discovery_metadata.get("match_score", 0.0)

        return self.approval_queue.submit(quote)


# ---------------------------------------------------------------------------
# Default orchestrator: prefer LangGraph when available
# ---------------------------------------------------------------------------
try:
    from langgraph_orchestrator import LangGraphQuoteOrchestrator as DefaultOrchestrator
except ImportError:
    DefaultOrchestrator = SupervisorAgent  # type: ignore[misc,assignment]
