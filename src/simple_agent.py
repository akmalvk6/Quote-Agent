        ...
        if self._tools_map is None:
            try:
                self._tools_map = {
                    ...
                    "find_similar_product": globals()["find_similar_product"]
                }
...
OUT_DIR    = _PROJECT_ROOT / os.getenv("QUOTES_OUT_DIR", "quotes")
APPROVAL_DIR = _PROJECT_ROOT / os.getenv("APPROVAL_QUEUE_DIR", "approval_queue")
...
def ensure_data():
    if not PRODUCTS_CSV.exists():
        pd.DataFrame([
            {"sku":"CH-100","name":"Office Chair","unit_price":1500,"tier":"standard","category":"Seating","description":"Ergonomic office chair with lumbar support and adjustable armrests","dimensions":"65x65x120cm","features":"lumbar support;adjustable height;360 swivel;breathable mesh","use_cases":"office workspace;home office;computer work;long hours sitting","keywords":"office chair;desk chair;ergonomic chair;computer chair;swivel chair"},
            {"sku":"TB-200","name":"Conference Table","unit_price":12000,"tier":"premium","category":"Tables","description":"Premium conference table for boardroom meetings and team collaboration","dimensions":"300x120x75cm","features":"cable management;modular design;scratch resistant;seats 12","use_cases":"boardroom meetings;team collaboration;client presentations;training rooms","keywords":"conference table;meeting table;boardroom table;large table;team table"},
            {"sku":"DS-300","name":"Developer Desk","unit_price":8000,"tier":"standard","category":"Tables","description":"Height-adjustable standing desk for developers and creative professionals","dimensions":"160x80x75-125cm","features":"motorized height adjust;cable tray;monitor arm mount;anti-fatigue mat","use_cases":"software development;study room;student workspace;home office;computer desk","keywords":"developer desk;standing desk;study table;student desk;work desk;computer desk;learning desk"},
            {"sku":"ST-400","name":"Visitor Stool","unit_price":900,"tier":"basic","category":"Seating","description":"Compact stacking stool for reception and waiting areas","dimensions":"45x45x65cm","features":"stackable;lightweight;non-slip feet;easy clean","use_cases":"reception areas;waiting rooms;temporary seating;event seating;break rooms","keywords":"visitor stool;stacking stool;waiting chair;reception seat;guest chair;bar stool"},
...
product_discovery = ProductDiscovery(PRODUCTS_CSV, DATA_DIR / "vector_store")
...
def find_similar_product(query: str) -> dict:
    """Find the closest matching products from the catalog using semantic search.

    Use this tool when price_lookup returns found=False. It performs
    RAG-based similarity search against the product catalog and returns
    confidence-tiered results.

    Args:
        query: The product name or description the user asked for

    Returns:
        Dictionary with match_type ('exact', 'auto', 'ask_user', 'no_match'),
        candidates list with scores, and a human-readable message.
    """
    with observability.span("tool.find_similar_product", query=query):
        return product_discovery.find_or_match(query, repository)
...
def quote_generator(customer: str, items_json: str, terms: str = "Standard T&C apply.", match_metadata_json: str = "") -> dict:
    ...
    Args:
        ...
        match_metadata_json: Optional JSON string with product discovery metadata,
            e.g. '{"requested_product":"study table","matched_product":"Developer Desk","match_score":0.92}'
    ...
    with observability.span("tool.quote_generator", customer=customer):
        ...
        # Embed product discovery metadata when semantic fallback was used
        if match_metadata_json:
            try:
                match_meta = json.loads(match_metadata_json)
                if match_meta.get("requested_product"):
                    quote["requested_product"] = match_meta["requested_product"]
                    quote["matched_product"] = match_meta.get("matched_product", "")
                    quote["match_score"] = match_meta.get("match_score", 0.0)
            except json.JSONDecodeError:
                pass
...
tools = [price_lookup, discount_calculator, historical_match, customer_memory, guardrail_agent, quote_generator, find_similar_product]
...
- quote_generator(customer: str, items_json: str, terms: str = "Standard T&C apply.", match_metadata_json: str = "") -> dict
- find_similar_product(query: str) -> dict
...
Step 3: Call price_lookup("product name") to get pricing
  Step 3a: If price_lookup returns found=True, continue to Step 4 with the matched product.
  Step 3b: If price_lookup returns found=False, call find_similar_product("product name").
  Step 3c: Check the match_type in the result:
    - If match_type is "exact" or "auto":
      Tell the user: "No exact match for 'X'. The closest product is 'Y' (Z% match)."
      Then call price_lookup(matched_product_name) and continue to Step 4.
    - If match_type is "ask_user":
      Present the candidates list to the user exactly as shown in the message field.
      DO NOT generate a quote. Wait for the user to choose a product.
    - If match_type is "no_match":
      Tell the user no matching product was found. Show available products.
      DO NOT generate a quote.
Step 4: Call discount_calculator(price, quantity, "regular" or "preferred")
Step 5: Call guardrail_agent(request_text, items_json, discount_pct, total)
Step 6: Call quote_generator(customer_name, items_json, terms, match_metadata_json)
  - If a product was found via find_similar_product, pass match_metadata_json with:
    '{"requested_product":"original","matched_product":"matched","match_score":0.XX}'
  - If the product was an exact match, leave match_metadata_json empty.

CRITICAL RULES:
- NEVER invent products that do not exist in the catalog.
- NEVER auto-generate a quote when match_type is "ask_user" — always ask the user first.
- NEVER generate a quote when match_type is "no_match".
- DO NOT generate formatted quotes as text. You MUST use quote_generator.
- Quotes are saved to an approval queue and require human approval before sending.
