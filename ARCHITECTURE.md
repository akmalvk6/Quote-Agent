...
graph TD
    ...
    subgraph FS [Data Storage]
        ...
        LOG[Quote Logs]
        CHROMA[ChromaDB Product Vectors]
    end
```
...
| **Product Discovery** | ChromaDB + Gemini Embeddings | Semantic product search with confidence-tiered matching | ChromaDB store |
...
3. **Tool Execution:** Sequential tool calls:
   ...
   - `find_similar_product()` - Semantic product fallback if exact match fails
...
1. **Exact Match:** Agent calls `price_lookup()` to find the product
2. **Semantic Fallback:** If not found, calls `find_similar_product()`
3. **Confidence Tiers:**
   - Score ≥ 0.85 → Auto-select the closest product
   - Score 0.70–0.84 → Present candidates, ask user to choose
   - Score < 0.70 → No match found, reject
4. **Quote Generation:** Proceeds with the matched product, logging discovery metadata
...
│   ├── ingest_products.py       # Product ingestion CLI
